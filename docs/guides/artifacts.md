...
| Image dimensions / MIME sniff | `pillow` — `pip install "lazycrawler[image] @ git+https://github.com/selvaz/LazyCrawler.git"` | dims omitted, MIME from magic bytes only |
| PDF tables / images | `pip install "lazycrawler[pdf] @ git+https://github.com/selvaz/LazyCrawler.git"` | PDFs yield no artifacts |
