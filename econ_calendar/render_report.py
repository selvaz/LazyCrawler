...
def _enrichment_block(r: dict) -> str:
    ...
    if r.get("technical_source"):
        parts.append(
            f'<p class="generated">Agency release cited: '
...
def _card(r: dict) -> str:
    ...
    period = (
        f' <span class="release-period">({_e(r["reference_period"])})</span>'
    ...
    for label, value in (("consensus", atteso), ("previous", r["previous"])):
        if value:
            figures.append(
                f'<div class="figure"><b>{label}</b><span class="mono">{_e(value)}</span></div>'
            )
    parts.append(f'<div class="release-figures">{"".join(figures)}</div>')

    lo, hi = r.get("consensus_low"), r.get("consensus_high")
    disperse = (
        lo is not None and hi is not None and abs(lo - hi) > 0.02 * max(abs(lo), abs(hi), 1e-9)
    )
    if disperse:
        parts.append(
    ...
    if r["values_agree"] is False:
        parts.append(
            f'<div class="warn">The {r["n_sources"]} sources disagree on the published value.</div>'
        )
...
def _summary_block(s) -> str:
    ...
    parts.append(
        '<p class="by">Written by claude-sonnet from the figures and coverage '
...
def render_html(day: date, rows: list[dict], consensus_source: str, summary=None) -> str:
    ...
    body = (
        f'<div class="release-grid">{"".join(_card(r) for r in rows)}</div>'
        if rows
        else '<p class="empty-note">No watchlist release on this day.</p>'
    )
