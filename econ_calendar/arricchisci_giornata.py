...
def save(con, event: dict, result, run_id: str | None, attempts: int = 1) -> str | None:
    ...
    con.execute(
        ...
        [event["event_id"], datetime.now(timezone.utc),
         "deepseek-v4-flash", "claude-sonnet (reasoning medium)", attempts,
         json.dumps([c.model_dump() for c in p.commentary], ensure_ascii=False),
         len(p.commentary), p.drivers, p.components, p.technical_source,
         p.not_found, run_id],
    )
    return via
...
def enrich_day(con, day: date, *, news_dir, news_db=None, regenerate=False,
    ...
    for e in events:
        ...
        try:
            ...
            costo = (f"  [{ricerche_fatte()} searches, "
                     f"{(datetime.now(timezone.utc) - partito).total_seconds():.0f}s]")
            if via is None:
                failed += 1
                print(f"      no usable output, not even after reshaping; skipped{costo}",
                      flush=True)
                continue
            p = result.payload if via == "direct" else salvage(result)[0]
            recuperato = {"direct": "", "json": " (json recovered from text)",
                          "haiku": " (prose reshaped by haiku)"}[via]
            print(f"      {len(p.commentary)} commentary{recuperato}"
